...
**改訂（2026-06-12 PM裁定）**: engine/template/events のフル再編は本リポジトリでは行わない。本リポジトリは Cloudflare Pages がルート直配信しており、配信パスの移動は「公開URL不変」制約（リスク1）と正面衝突する。フル構造は Phase 5 の**新規公開リポジトリ側のレイアウト**として実現し、本リポジトリの Phase 4 は次に縮小する:
1. `template/` 新設 — 正本テンプレ一本化（master/ と test/master/ に分散する template CSV・sample_csv の整理統合）。init_tournament.py の出力先を template/ に変更（凍結領域 master/ 上書き事故の恒久対策）
2. docs 統合 — SPEC.md / operation-handover.md の生きている内容を ARCHITECTURE.md へ統合し、両ファイルは廃止予告ヘッダ+ポインタ化
3. 配信中ファイル（index/js/css/data/admin/staff/_headers/_redirects）は**1バイトも動かさない**

目的: プラグインとして配布できるディレクトリ構造に組み替える。（旧記述・新リポジトリ側レイアウトの参考として保持）
